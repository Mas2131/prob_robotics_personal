function transition_probability_matrix = transitionModel(map_, row_from_, col_from_, control_input_)
  ...
  #against each other cell and itself
	for row_to = 1:map_rows
      ...
      translation_cols = col_to - col_from_; #we are checking if the cell we want to move to is more than 1 step from us
      ...
      #compute target robot position according to input ##based on the commands we get from the users. Orazio moves according to the commands
      ...
      endswitch
	    ...
	    #check if the desired motion is infeasible we need to check if the position is inside the map or occupied
      ...
      switch (control_input_)
        case MOVE_UP 
          if (translation_rows     == -1 && translation_cols ==  0) transition_probability_matrix(row_to, col_to) = 0.8; #we are commanding of going in a certain direction, the movement is feasible, we have no noise: we WILL go up. If we have noise, the probability that it will go up is 0.8 witha a drift prob 0.2
