...
global state_belief = ones(rows(map), columns(map))*belief_initial_value; ##our goal is to find position, so point on a 2 by 2 grid
...
#---------------------------------- FILTERING ----------------------------------
  ...
  state_belief = zeros(map_rows, map_cols);
	for row = 1:map_rows
		for col = 1:map_cols
      state_belief += transitionModel(map, row, col, control_input)*state_belief_previous(row, col); ##we want to predict the belief, this is basically writing in code the formula on page 26 on the slides
		endfor
	endfor

  #UPDATE robot position belief and COMPUTE the normalizer 
  ##to update the belief we need observations
	inverse_normalizer = 0;
	for row = 1:map_rows
		for col = 1:map_cols
    ##we will just write the formulas on page 27
